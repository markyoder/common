...
def pca1(theta=math.pi/6., a=1.0, b=.5, x0=0., y0=0., N=1000):
	...
	XY = [[x0 + a*Rx.random(), y0 + b*Ry.random()] for n in xrange(N)]
	...
	plt.figure(1)
	plt.clf()
	plt.plot(*zip(*pcas.Y), marker='.', ls='', color='g')
...
class PCA_transform(object):
	...
	def calc_pca(self, data_in=None, do_return=False):
		...
		cov_normed = numpy.dot(numpy.array(zip(*data_centered)),numpy.array(data_centered))/(float(len(data_centered))-1.)
		#
	...
	def plot_data(self, fignum=0, axes=[0,1]):
		...
		plt.plot(*zip(*vx), color='r', lw=1.5, ls='-', marker='s')
		plt.plot(*zip(*vy), color='g', lw=1.5, ls='-', marker='s')
		#
...
def pca_test2(theta=math.pi/6., N=1000, x0=0., y0=0., fignum=0):
	...
	plt.figure(fignum)
	plt.clf()
	plt.plot(*zip(*my_data), marker='.', color='b', zorder=2, ls='')
	plt.plot(*zip([0,.0], ax_x), marker='s', ls='-', lw=2, color='r')
	plt.plot(*zip([0,.0], ax_y), marker='s', ls='-', lw=2, color='g')
	#
...
def yoda_pca(data_in):
	...
	my_cov = numpy.dot(numpy.array(zip(*centered)),numpy.array(centered))/(float(len(data_in))-1.)
	#
...
def rotate_ccw(v,theta, x0=0., y0=0.):
	...
	if hasattr(v[0], '__len__'):
		...
		tail = v[0]
		tip  = [v[-1][j] - v[0][j] for j in xrange(len(v[0]))]
		#
...
def make_test_data_gen(thetas=[math.pi/6.], N=100, X0=[0., 0.], delta_X=[2.,1.]):
	...
	XY = [[x0 + dx*r.random() for x0,dx,r in zip(X0, delta_X, Rs)] for n in xrange(N)]
	#
