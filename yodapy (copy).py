...
class linefit:
	...
	def doFit(self, a=None, b=None, xmin=None, xmax=None, thisdatas=None):
		if thisdatas==None: thisdatas=self.datas
		# do a linear fit:
		# a,b parameters are first guesses.
		# if they are None, guess starting prams:
		...
		# note: args are (y, x, wt)
		#plsq=spo.leastsq(self.linRes, p, args=(scipy.array(self.datas[1]), scipy.array(self.datas[0]), scipy.array(self.datas[2])), full_output=1)
		plsq=spo.leastsq(self.activeRes, p, args=(scipy.array(Y), scipy.array(X), scipy.array(W)), full_output=1)
		#print "fit done. sum error..."
		for sig in self.errs:
			self.totalVar+=sig*sig
		#self.Ndof=len(thisdatas[0])-len(p)
		self.AIC=self.totalVar+len(p)
		self.Ndof=len(X)-len(p)
		self.dataLen=len(X)
		self.a=plsq[0][0]
		self.b=plsq[0][1]
		...
		self.vara=(1.0/self.delta)*sum(aXX*aW)
		self.varb=(1.0/self.delta)*sum(aW)	# note that w=1/sig^2 in most texts, as per gaussian stats. this is more general than that, and it might make a big fat
...
def updateANSS2SQL(catID=523):
	...
	# this function assumes all existing data is correct and complete.
...
class eqcatalog:
	...
	def initialize(self, inData=[]):
		...
		inData=None
		self.sqlPort=3306
		self.sqlHost='localhost'	# or '127.0.0.1'
...
#[0,0], [2,0], [4,4], [2,6], [0,4]			
	...
	def plotCatMap(self, catalog=None, doShow=True, doSave=False, saveName='catalogPlot.png', epicenter=None, legendLoc='upper left', doCLF=True, eqicon='b,'):
		if catalog==None: catalog=self.cat
		...
		#
		if doShow>=1: fnum=doShow
		f0=plt.figure(int(doShow))	
		if doCLF: plt.clf()
		#		
		#set up map:
		#llr=self.getLatLonRange(catalog)	# latLonRange
		llr=self.getLatLonRange(self.cat)	# latLonRange
		llr[0][0]-=2.0
		llr[0][1]-=2.0
		llr[1][0]+=2.0
		llr[1][1]+=2.0
	...
	def testMap(self):
		...
		m = Basemap(llcrnrlon=lllon, llcrnrlat=lllat, urcrnrlon=urlon, urcrnrlat=urlat, resolution='f', projection='tmerc', lon_0=lon0, lat_0=lat0)
		m.drawcountries()
		m.drawrivers()
	...
	def plotCatsMap(self, catalogses=None, maincat=0, doShow=True, doSave=False, saveName='catalogPlot.png', epicenter=None, legendLoc='upper left', maincatname='full cat'):
		...
		catmap=Basemap(llcrnrlon=llr[0][1], llcrnrlat=llr[0][0], urcrnrlon=llr[1][1], urcrnrlat=llr[1][0], resolution ='c', projection='tmerc', lon_0=cntr[1], lat_0=cntr[0])
	...
	def getCatFromSQL(self, startDate=dtm.datetime(1999,9,28, 17,15,24), endDate=dtm.datetime(2009,9,28, 17,15,24), lats=[32.0, 37.0], lons=[-125.0, -115.0], minmag=2.0, catalogName='Earthquakes', catalogID=523, ordering='asc'):
		...
		import MySQLdb
		#
		sqlHost = self.sqlHost #'localhost'
		sqlUser = 'myoder'
		sqlPassword = 'yoda'
		#sqlPort = 3306
	...
	def getIntervals(self, catList, winLen):
		...
		return intervals
	#
	def plotIntervals(self, intervals=[10, 100, 1000], minmag=2.0, catalog=None, fignum=0, dtmlatlonmagCols=[0,1,2,3], plotDates=[None, None]):
		...
		for wlen in intervals:
			...
			#
			ax1.plot(map(operator.itemgetter(0), shockints[-1]), scipy.array(map(operator.itemgetter(1), shockints[-1]))/float(wlen), '-', label='$N=%d$' % wlen, lw=1.0)
